...
> = {
  ...
  PREPARING: {
    ...
    className: "border-sky-400 bg-sky-50 text-sky-900",
  },
  UNAVAILABLE: {
    label: "Unavailable",
...
export function PublicFloorMap({ floor }: { floor: PublicFloorView }) {
  ...
  return (
    <section className="mt-8">
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-stone-200 bg-stone-100 p-3 shadow-inner">
        <div
        ...
        >
          ...
          {tables.map((table) => {
            ...
            return (
              <div
                key={table.id}
                className={`absolute grid place-items-center overflow-hidden border-2 px-1 text-center shadow-sm ${meta.className}`}
                style={{
                ...
                aria-label={`${table.label ?? "Table"}, ${table.capacity ?? 0} seats, ${meta.label}`}
                title={`${table.label ?? "Table"} · ${table.capacity ?? 0} seats · ${meta.label}`}
              >
