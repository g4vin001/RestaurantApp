...
export async function joinCustomerWaitlist(
  ...
  if (partyName.length < 2 || partyName.length > 80) return { error: "Enter a party name between 2 and 80 characters." };
  if (!Number.isInteger(partySize) || partySize < 1 || partySize > 30) return { error: "Party size must be between 1 and 30." };

  const restaurant = await prisma.restaurant.findUnique({ where: { slug }, select: { id: true, walkInAvailability: true } });
  if (!restaurant) return { error: "Restaurant not found." };
  if (restaurant.walkInAvailability === "PAUSED") return { error: "This restaurant is not accepting new walk-ins right now." };
  ...
  const promisedWaitMinutes = Math.max(0, publicView?.estimatedWaitMinutes ?? 15);
  ...
  await prisma.restaurant.update({ where: { id: restaurant.id }, data: { lastOperationalUpdateAt: new Date() } });
