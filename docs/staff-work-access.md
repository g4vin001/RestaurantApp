...
The email match establishes **who the person is**. The restaurant's shared 4-digit staff PIN is a second gate used only to enter work mode. It is not an employee identifier and does not create a separate authentication account.

On the first successful clock-in, Halina may create or reactivate an internal `RestaurantMembership` with role `STAFF` and link it to the `StaffMember`. That row is a tenancy/audit relationship used by the existing transactional operations layer. It is **not** a login credential and does not let someone enter `/ops` without a valid clocked-in `StaffWorkSession`.
...
6. the staff whitelist email still matches the authenticated account's current verified email;
7. the linked internal `STAFF` membership is still valid; and
8. the current `StaffRole.permissions` allows the requested operation.
...
Staff roles may grant only the restricted operations permission ceiling:
...
The default Floor Staff, Host, and Shift Lead roles use subsets of that ceiling, and custom roles remain supported. `StaffRole.permissions` is authoritative. The legacy `permissionPreset` remains only as a compatibility/display fallback.

No staff role grants `/manager`, Team, floor-plan editing, analytics, restaurant settings, owner onboarding, or tenant switching. Manager workspace authorization remains `RestaurantMembership` with `OWNER` or `MANAGER`.
...
- assign a default or custom staff role;
...
Changing the whitelist identity, disabling access, deactivating, or archiving a staff record terminates active work sessions. Deactivation/archival also disables the internal `STAFF` membership so it cannot be used independently of the next successful PIN clock-in.

## Operational writes

Staff operations are tenant-scoped from the validated work session, never from a client-supplied restaurant ID. After validation, the existing `executeOperationsCommand` layer receives the linked `STAFF` membership so current transaction, idempotency, optimistic-concurrency, audit, seating-assignment, and real-time behavior remains intact.
...
The source migration `20260903130000_staff_whitelist_clock_in` adds the whitelist, hashed PIN, work-session, and PIN-attempt fields/tables. It also safely backfills normalized email for existing active staff where possible. Committing the migration does not by itself apply it to an environment; apply it only through the project's reviewed database migration process.
